...
class AopCertification
{
    ...
    public static function verifySignature($alipayCert, $rootCert)
    {
        ...
        $checkSign       = self::isCertSigner(
            $alipayCertArray[0] . "-----END CERTIFICATE-----",
            $alipayCertArray[1] . "-----END CERTIFICATE-----"
        );
        if (!$checkSign) {
            $checkSign = self::isCertSigner(
                $alipayCertArray[1] . "-----END CERTIFICATE-----",
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                if ($issuer == $subject) {
                    $checkSign = self::isCertSigner(
                        $alipayCertArray[1] . "-----END CERTIFICATE-----",
                        $rootCertArray[$i] . "-----END CERTIFICATE-----"
                    );
                    return $checkSign;
                }
            }
    ...
    /**
     * 向上构造证书链
     * @param array $subjectMap 主题和证书的映射
     * @param array $certChain 证书链
    ...
    /**
     * 向下构造证书链
     * @param array $issuerMap 签发者和证书的映射
     * @param array $certChain 证书链
