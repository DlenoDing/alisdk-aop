...
class AopCertClient
{
    ...
    protected $alipaySdkVersion = "alipay-sdk-PHP-4.20.175.ALL";
    ...
    public function __construct()
    {
        ...
        if (func_num_args() == 1 && func_get_arg(0) instanceof AlipayConfig) {
            ...
            $alipayPublicContent      = !$this->checkEmpty(
                $config->getAlipayPublicCertContent()
            ) ? $config->getAlipayPublicCertContent() : file_get_contents($config->getAlipayPublicCertPath());
            $this->alipayrsaPublicKey = $this->getPublicKeyFromContent($alipayPublicContent);

            $appCertContent  = !$this->checkEmpty($config->getAppCertContent()) ? $config->getAppCertContent(
            ) : file_get_contents($config->getAppCertPath());
            $this->appCertSN = $this->getCertSNFromContent($appCertContent);

            $rootCertContent        = !$this->checkEmpty($config->getRootCertContent()) ? $config->getRootCertContent(
            ) : file_get_contents($config->getRootCertPath());
    ...
    public function getRootCertSN($certPath)
    {
        ...
        for ($i = 0; $i < count($array) - 1; $i++) {
            ...
            if ($ssl[$i]['signatureTypeLN'] == "sha1WithRSAEncryption" || $ssl[$i]['signatureTypeLN'] == "sha256WithRSAEncryption") {
                if ($SN == null) {
                    $SN = md5(
                        AopCertification::array2string(array_reverse($ssl[$i]['issuer'])) . $ssl[$i]['serialNumber']
                    );
                } else {
                    $SN = $SN . "_" . md5(
                            AopCertification::array2string(array_reverse($ssl[$i]['issuer'])) . $ssl[$i]['serialNumber']
    ...
    public function splitCN($cont, $n = 0, $subnum = 1, $charset = "gbk")
    {
        //$len = strlen($cont) / 3;
        $arrr = array();
    ...
    public function subCNchar($str, $start = 0, $length = 1, $charset = "gbk")
    {
    ...
    /**
     * 生成用于调用收银台SDK的字符串
     * @param mixed $request SDK接口的请求参数对象
    ...
    /**
     * 页面提交执行方法
     * @param mixed $request 跳转类接口的request
    ...
    {
        $sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='" . $this->gatewayUrl . "?charset=" . trim(
                $this->postCharset
            ) . "' method='POST'>";
    ...
    public function checkResponseSign($request, $signData, $resp, $respObject)
    {
        if (!$this->checkEmpty($this->alipayPublicKey) || !$this->checkEmpty($this->alipayrsaPublicKey)) {
            if ($signData == null || $this->checkEmpty($signData->sign) || $this->checkEmpty(
            ...
            if (!$this->checkEmpty($responseSubCode) || ($this->checkEmpty($responseSubCode) && !$this->checkEmpty(
                        $signData->sign
                    ))) {
                ...
                if (!$checkResult) {
                    ...
                    $requestUrl = $this->gatewayUrl . "?";
                    foreach ($sysParams as $sysParamKey => $sysParamValue) {
                        $requestUrl .= "$sysParamKey=" . urlencode(
                                $this->characet($sysParamValue, $this->postCharset)
                            ) . "&";
                    }
                    $requestUrl = substr($requestUrl, 0, -1);
                    //发起HTTP请求
